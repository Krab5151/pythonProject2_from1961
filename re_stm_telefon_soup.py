...
# soup = BeautifulSoup(html, "html.parser")
#
# # Извлекаем заголовок адреса
# title = soup.find("p", class_="main-title").text
#
# # Извлекаем сам адрес (следующий <p> после заголовка)
# address = soup.find("p", class_="main-title").find_next_sibling("p").text.strip()
#
# # Извлекаем время работы (ещё один <p> после адреса)
# work_time = soup.find("p", class_="main-title").find_next_sibling("p").find_next_sibling("p").text.strip()
#
# print(" Адрес:", title, "-", address)
